	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				...
				{field:'series',title:'所属专业',width:150, sortable: true,
					formatter:function(value,index,row){
						for(var i=0;i<seriesList.length;i++){
							if(seriesList[i].id == value){
								return seriesList[i].name;
							}
						}
						return value;
					}
				},
 		        {field:'year',title:'创办时间',width:300},
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "如果年级下存在班级信息则无法删除，须先删除年级下属的所有班级信息！<span style='color:red;'>并且该操作不可逆，请慎重！</span", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "delete",
	    ...
	    $("#addDialog").dialog({
	  	...
	  	$("#search-btn").click(function(){
	  		$('#dataList').datagrid('reload',{
				...
				series:$("#search-series-id").combobox('getValue')
	  		});
	  	});
	});
	</script>
...
<body>
	...
	<div id="toolbar">
		...
		<div>
			...
			所属专业：
			<select id="search-series-id" class="easyui-combobox" style="width: 150px;">
   		...
   		<form id="addForm" method="post">
	    	<table id="addTable" cellpadding="8">
				...
				<tr >
					<td>所属专业:</td>
					<td>
						<select id="add_series"  class="easyui-combobox" style="width: 200px;" name="series" data-options="required:true, missingMessage:'请选择所属年级'">
							<c:forEach items="${seriesList}" var="series">
								<option value="${series.id }">${series.name }</option>
							</c:forEach>
						</select>
    	...
    	<form id="editForm" method="post">
	    	...
	    	<table id="editTable" border=0 cellpadding="8" >
				...
				<tr >
					<td>所属专业:</td>
					<td>
						<select id="add_update"  class="easyui-combobox" style="width: 200px;" name="series" data-options="required:true, missingMessage:'请选择所属专业'">
							<c:forEach items="${seriesList}" var="series">
								<option value="${series.id }">${series.name }</option>
							</c:forEach>
						</select>
